...
// Verificar PIN do vendedor (publico)
router.post('/vendedores/:slug/pin', (req, res) => {
  const { pin } = req.body;
  if (!pin) return res.status(400).json({ erro: 'PIN obrigatório' });
  const v = db.prepare('SELECT id, pin FROM vendedores WHERE slug=? AND ativo=1').get(req.params.slug);
  if (!v) return res.status(404).json({ erro: 'Vendedor não encontrado' });
  // Se nao tem PIN cadastrado, qualquer coisa passa (para nao travar quem nao configurou)
  if (!v.pin || v.pin === pin) return res.json({ ok: true });
  return res.status(401).json({ erro: 'PIN incorreto' });
});

router.post('/vendedores', auth, (req, res) => {
  const { nome, email, pin } = req.body;
  if (!nome) return res.status(400).json({ erro: 'Nome obrigatório' });
  const slug = nome.toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g,'')
    .replace(/\s+/g,'').replace(/[^a-z0-9]/g,'');
  try {
    const r = db.prepare('INSERT INTO vendedores (nome, slug, email, pin) VALUES (?,?,?,?)').run(nome, slug, email||null, pin||null);
...
router.put('/vendedores/:id', auth, (req, res) => {
  const { nome, email, ativo, pin } = req.body;
  const v = db.prepare('SELECT * FROM vendedores WHERE id=?').get(req.params.id);
  if (!v) return res.status(404).json({ erro: 'Não encontrado' });
  db.prepare('UPDATE vendedores SET nome=?, email=?, ativo=?, pin=? WHERE id=?')
    .run(nome||v.nome, email||v.email, ativo!==undefined?(ativo?1:0):v.ativo, pin!==undefined?pin:v.pin, req.params.id);
  res.json({ ok: true });
});
...
router.put('/produtos/:id', auth, (req, res) => {
  ...
  db.prepare(`UPDATE produtos SET nome_completo=?, nome_dash=?, categoria=?,
...
router.get('/registros', auth, (req, res) => {
  ...
  const registros = db.prepare(`
    ...
    ${where}
    ORDER BY r.data_registro DESC, r.criado_em DESC LIMIT 500
  `).all();
  res.json(registros);
});
...
router.get('/comercial/ranking', auth, (req, res) => {
  ...
  const ranking = db.prepare(`
    SELECT
      ...
      COALESCE(SUM(r.qtd_leads),0) as total_leads,
      COALESCE(SUM(r.qtd_conversoes),0) as total_conversoes,
      COALESCE(SUM(r.valor_total),0) as faturamento,
      CASE WHEN COALESCE(SUM(r.qtd_leads),0)>0
        THEN ROUND(COALESCE(SUM(r.qtd_conversoes),0)*100.0/SUM(r.qtd_leads),1)
