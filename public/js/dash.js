  ...
  document.getElementById('webhook-docs').innerHTML = `
    <div class="card">
      ...
      <div class="card-b" style="font-size:12px;">
        ...
        <div style="background:var(--bg-3);border:1px solid var(--b0);border-radius:6px;padding:10px;font-family:'Geist Mono',monospace;font-size:10.5px;line-height:1.9;margin-bottom:10px;">
          { "event": "order_approved",<br>&nbsp;&nbsp;"data": { "product": { <span style="color:var(--dn);">"id"</span>: <span style="color:var(--up);">"prod_ttshop01"</span> }}}
        </div>
    ...
    <div class="card">
      ...
      <div class="card-b" style="font-size:12px;">
        ...
        <div style="background:var(--bg-3);border:1px solid var(--b0);border-radius:6px;padding:10px;font-family:'Geist Mono',monospace;font-size:10.5px;line-height:1.9;margin-bottom:10px;">
          { "tipo": "purchase.approved",<br>&nbsp;&nbsp;"venda": { <span style="color:var(--dn);">"produto_id"</span>: <span style="color:var(--yn);">11204</span> }}
        </div>
    ...
    <div class="card">
      ...
      <div class="card-b" style="font-size:12px;">
        ...
        <div style="background:var(--bg-3);border:1px solid var(--b0);border-radius:6px;padding:10px;font-family:'Geist Mono',monospace;font-size:10.5px;line-height:1.9;margin-bottom:10px;">
          { "event_type": "sale_approved",<br>&nbsp;&nbsp;"sale": { <span style="color:#a78bfa;">"product_id"</span>: <span style="color:var(--up);">"LL-44201"</span>, "amount": 99700 }}
        </div>
...
async function testWebhook(plat) {
  ...
  const r = await fetch(`/api/webhooks/${plat}`,{
    method:'POST',
    headers: authHeaders(),
...
async function loadConfig() {
  ...
  if (admins) {
    document.getElementById('admins-list').innerHTML = admins.length
      ...
      : '<div style="padding:20px;text-align:center;color:var(--t2);font-size:12.5px;">Nenhum admin.</div>';
  }
  if (vendedores) {
    document.getElementById('vendedores-list').innerHTML = vendedores.length
      ? vendedores.map(v=>`
      <div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--b0);">
        ...
        <div style="flex:1;"><div style="font-size:12.5px;font-weight:500;">${v.nome}</div>
          <div style="font-size:11px;color:var(--t2);">${location.origin}/<strong style="color:var(--acc);">${v.slug}</strong></div></div>
        <button class="btn btn-ghost btn-sm" onclick="copyLink('${v.slug}')"><i class="fas fa-copy"></i></button>
      </div>`).join('')
      : '<div style="padding:20px;text-align:center;color:var(--t2);font-size:12.5px;">Nenhum vendedor cadastrado.</div>';
  }
}
...
async function salvarProduto() {
  ...
  const r = await fetch('/api/produtos', { method:'POST', headers: authHeaders(), body: JSON.stringify(body) });
  const d = await r.json();
...
async function salvarVendedor() {
  ...
  const r = await fetch('/api/vendedores', { method:'POST', headers: authHeaders(), body: JSON.stringify(body) });
  const d = await r.json();
  if (!r.ok) { toast(d.erro||'Erro','err'); return; }
  toast(`Vendedor criado! Link: ${location.origin}/${d.slug}`,'ok');
...
async function salvarAdmin() {
  ...
  const r = await fetch('/api/auth/admins', { method:'POST', headers: authHeaders(), body: JSON.stringify(body) });
  const d = await r.json();
  if (!r.ok) { toast(d.erro||'Erro','err'); return; }
...
async function loadMe() {
  const admin = JSON.parse(localStorage.getItem('dash_admin') || '{}');
  if (admin?.nome) {
    document.getElementById('u-nome').textContent = admin.nome;
    document.getElementById('u-av').textContent   = admin.nome.substring(0,2).toUpperCase();
  } else {
